  ...
  memoize(
    (node: cytoscape.NodeSingular): Aggregated<Image> => {
      ...
      aggregated['bounds-expansion'] = [
        Math.max(...(aggregated['bounds-expansion'] as number[]), 0),
      ];
...
export const CROSS = memoize(
  (properties: Properties, { width, height }: DrawerParameters): Image => {
...
export const OMMITED_ICON = memoize(
  (properties: Properties) => {
    ...
    return svgStr(
      `<line x1="2.5" y1="3" x2="4.5" y2="7" stroke-width="1.5" stroke-linecap="round" stroke="${s}"/><line x1="5.5" y1="3" x2="7.5" y2="7" stroke-width="1.5" stroke-linecap="round" stroke="${s}"/>`,
      10,
      10
    );
  },
