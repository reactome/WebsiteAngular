...
export class DiagramComponent implements AfterViewInit, OnDestroy {
  ...
  onPopupNavigate(event: { stId: string; kind: EntityPopupTab }): void {
    ...
    // Setting the signal to the value it already holds is a no-op, so the
    // diagram would sit still. That happens on the popup's title, which comes
    // back to an entity whose right-click already selected it -- and a button
    // that does nothing is what sent us here in the first place. Re-centre
    // directly in that case.
    if (this.state.select() === event.stId) {
      this.avoidSideEffect(() =>
        this.cys.forEach((cy) => this.select(event.stId, cy))
      );
      return;
    }
