...
export class DataStateService {
  ...
  _currentPathway = rxResource({
    ...
    stream: (params) =>
      this.fetchEnhancedData<Pathway>(params.params, {
  ...
  private _selectedElement = rxResource({
    ...
    stream: (params) =>
      !params.params.id
        ? of(undefined)
        : this.fetchEnhancedData<SelectableObject>(params.params.id, {
            includeDisease: params.params.summariseDisease === true,
          }),
  });
  ...
  selectedPathwayStId = linkedSignal<SelectionData, string | undefined>({
    ...
    computation: (
      ...
      previous?: { source: SelectionData; value: string | undefined }
    ) => {
      if (source.selectedElementLoading && previous) return previous.value;
      if (source.selectedElement && isPathway(source.selectedElement))
        return source.selectedElement.stId;
  ...
  flagResource = rxResource({
    ...
    stream: ({ params: { diagram, tokens, species } }) =>
      tokens.length === 0
  ...
  getReacfoamFlagging(query: string, species: string): Observable<Partial<FlaggingResult>> {
    return this.http
      .get<FireworksFlagResult>(`${CONTENT_SERVICE}/search/fireworks/flag`, {
  ...
  getDiagramFlagging(diagram: string, query: string): Observable<Partial<FlaggingResult>> {
    return this.http
      .get<DiagramFlagResult>(`${CONTENT_SERVICE}/search/diagram/${diagram}/flag`, {
  ...
  fetchEnhancedData<T extends DatabaseObject>(
