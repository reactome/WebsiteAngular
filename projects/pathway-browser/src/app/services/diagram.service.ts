...
export class DiagramService {
  ...
  public getNormalPathway(id: string): Observable<string> {
    return this.http
      .get(`${CONTENT_SERVICE}/data/query/${id}/normalPathway`, {
        responseType: 'text',
      })
  ...
  public getCHEBIStructure(
    ...
    return new Map<string, Promise<string | undefined>>(
      chebiIds.map((id) => [id, this.loadStructureSvg(parseInt(id))])
    );
  }
  ...
  public diagramFromData(
    ...
    console.assert(
      forwardArray.length === this.extraLine.size,
      'Some edge diagram have been lost because 2 segments are starting from the same point'
    );
    ...
    console.assert(
      backwardArray.length == this.reverseExtraLine.size,
    ...
    const dbIdToGraphEdge = new Map<number, Graph.Edge>(
      graph.edges.map((edge) => [edge.dbId, edge])
    );
     ...
     *
     */
    const edges: cytoscape.EdgeDefinition[] = diagram.nodes.flatMap((node) => {
      return node.connectors.map((connector) => {
        ...
        console.assert(
          d >= MIN_DIST,
          `${id}\t${diagram.displayName}\t${hasFadeOut}\tR-HSA-${reaction.reactomeId}\tR-HSA-${node.reactomeId}\thttps://newcurator.reactome.org/curatorgraph/PathwayBrowser/#/${id}&SEL=R-HSA-${reaction.reactomeId}&FLG=R-HSA-${node.reactomeId}\thttps://reactome-pwp.github.io/PathwayBrowser/${id}?select=${reaction.reactomeId}&flag=${node.reactomeId}`
        );
        ...
        if (!connector.isFadeOut) {
          ...
          replacement =
            replacement ||
            (
              posToNormalNode.get(pointToStr(node.position)) &&
              posToNormalEdge.get(pointToStr(reaction.position))
            )?.id;
          // console.log("Reaction edge", replacement)
        }
  ...
  private getEndpointData(
    ...
    from: Position,
    to: Position
  ): Partial<{ sourceEndpoint: string; targetEndpoint: string }> {
    if (
  ...
  private getRelativeSegmentsData(
