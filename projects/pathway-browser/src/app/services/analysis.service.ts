...
export class PaletteSummary {
  ...
  constructor(private data: StandardPalette | string[]) {
    if (typeof data === 'string') {
      ...
      // Different functions being called, though same name according to typescript
  ...
  getCssGradient(direction: 'top' | 'bottom' | 'left' | 'right') {
    return this.n === -1
      ...
      : `linear-gradient(to ${direction} in oklab, ${this.scale
          .colors(this.n)
          .map((c, i) => `${c} ${(i / this.n) * 100}%, ${c} ${((i + 1) / this.n) * 100}%`)
          .join(', ')})`;
  }
...
export class AnalysisService {
    ...
    (
      [
        ...Object.keys(chroma.brewer)
          ...
          .map(
            (name) =>
              [name, new PaletteSummary(name as StandardPalette)] as [
                StandardPalette,
                PaletteSummary,
              ]
          ),
        [
          'ancient',
          new PaletteSummary(['#1532b3', extract(this.style.properties.global.surface), '#e5e61d']),
        ],
        [
          'primary',
          new PaletteSummary([
      ...
      ] as [PaletteName, PaletteSummary][]
    ).map(([key, palette]) => [
      key,
      palette.noData(extract(this.style.properties.analysis.notFound)),
    ])
  );
  ...
  sampleIndex = linkedSignal({
    ...
    computation: ({ result, sample }) =>
      Math.max(
        // Avoid -1 value from indexOf
        (result && sample && result?.expression.columnNames?.indexOf(sample)) || 0,
        0
      ),
  });
    ...
    effect(() => {
      ...
      if (this.state.palette()) {
        const isValidPalette = [...validGroups.values()].some((validGroup) =>
          this.paletteGroups
            .find((group) => group.name === validGroup)
  ...
  loadDefaultExample(name: string): Observable<Analysis.Result> {
    return this.http
      .get(`assets/data/analysis-examples/${name}.tsv`, { responseType: 'text' })
      .pipe(switchMap((example) => this.analyse(example, { projectToHuman: true }, false)));
  }
  ...
  analyse(
    data: string,
    params?: Partial<Analysis.Parameters>,
    clearFilters = true
  ): Observable<Analysis.Result> {
    this.isLoading.set(true);
    return this.http
      .post<Analysis.Result>(
        `${environment.host}/AnalysisService/identifiers/${params?.projectToHuman ? 'projection' : ''}`,
        data,
        { params }
      )
      .pipe(
  ...
  analyseSpecies(
    ...
    return this.http
      .get<Analysis.Result>(
        `${environment.host}/AnalysisService/species/homoSapiens/${species.dbId}`,
        { params }
      )
      .pipe(
        tap((result) => this.result.set(result)),
  ...
  analyseFromUrl(
    ...
    return this.http
      .post<Analysis.Result>(
        `${environment.host}/AnalysisService/identifiers/url${params?.projectToHuman ? '/projection' : ''}`,
        url,
        { params }
      )
      .pipe(
  ...
  loadAnalysis(token?: string, params?: Partial<Analysis.Parameters>): Observable<Analysis.Result> {
    cleanObject(params || {});
    return this.http.get<Analysis.Result>(
      `${environment.host}/AnalysisService/token/${token || this.state.analysis()}`,
      { params }
    );
  }
  ...
  foundEntities(
    ...
    return this.http
      .get<Analysis.FoundEntities>(
        `${environment.host}/AnalysisService/token/${token || this.state.analysis()}/found/all/${pathway}`,
        {
          params: { resource },
        }
      )
  ...
  pathwaysResults(
    ...
    return this.http
      .post<Analysis.Pathway[]>(
        `${environment.host}/AnalysisService/token/${token || this.state.analysis()}/filter/pathways`,
        pathwayIds.join(','),
        {
          params: { resource },
        }
  ...
  getHitReactions(pathwayId: string, token: string, params?: Partial<Analysis.Parameters>) {
    ...
    return this.http.get<number[]>(
      `${environment.host}/AnalysisService/token/${token}/reactions/${pathwayId}`,
      { params }
    );
  }
  ...
  notFoundIdentifiersResource = rxResource({
    ...
    stream: ({ params }) =>
      params.token ? this.notFound(params.token, params.pagination) : of([]),
  });

  notFound(token: string, pagination: Pagination): Observable<NotFoundIdentifier[]> {
    return this.http.get<NotFoundIdentifier[]>(
      `${environment.host}/AnalysisService/token/${token}/notFound`,
      {
        params: pagination,
      }
    );
