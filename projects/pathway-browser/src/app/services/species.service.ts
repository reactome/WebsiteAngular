...
export class SpeciesService {
  ...
  constructor() {
    effect(() => {
      const newSpecies = this.abbreviationToSpecies().get(
        this.state.pathwayId()?.substring(2, 5) || ''
      );
    ...
    return this.getOrthologousMap(
      ...
      map((orthologousMap) => {
        for (const ancestor of ancestors.reverse()) {
          // Start from the end of ancestry, and gradually go up if there is no equivalent pathway
  ...
  private getOrthologousMap(
    identifiers: string[],
    speciesDbId: number
  ): Observable<OrthologousMap> {
    if (identifiers.length === 0) return of(new Map());
    const url = `${this._ORTHOLOGIES}${speciesDbId}`;
    return this.http
      .post<{ [p: string]: SelectableObject }>(
        url,
  ...
  getOrtholog(identifier: string | number | null, speciesDbID: number): Observable<Event | null> {
    ...
    return this.http.get<Event>(
      `${CONTENT_SERVICE}/data/orthology/${identifier}/species/${speciesDbID}`
    );
  }
