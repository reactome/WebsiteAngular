...
export class IconService {
  ...
  reactomeSubjectIcons: { [key: string]: { name: string; tooltip?: string; route: string } } = {
    ...
    ReferenceMolecule: {
      name: 'small-molecule',
      tooltip: 'Simple Entity',
      route: 'small-molecule',
    },
    ...
    CellLineagePath: {
      name: 'pathway',
      tooltip:
        'A collection of related Events describing development of a cell line.There events can be CellDevelopmentSteps or CellLineagePaths',
      route: 'pathway',
    },
    CatalystActivity: {
  ...
  fetchIcon(identifier: string): Observable<string | null> {
    return this.http
      .get<Search.Icon.Result>(`${CONTENT_SERVICE}/search/query?query=${identifier}&types=Icon`)
      .pipe(
        map((response) =>
