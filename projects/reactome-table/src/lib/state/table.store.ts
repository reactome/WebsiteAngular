...
export class TableStore extends ComponentStore<TableState> {
  ...
  readonly cleanData$ = this.select((state) => {
    ...
    if (state.numberOfColumns && state.numberOfRows) {
      ...
      table = table
        ...
        .map((row) => row.slice(origin.x, firstEmptyColIndex));
    }
  ...
  readonly range$ = this.select((state) => {
    ...
    return state.dataset
      ...
      .map((row) => row.slice(range.x.min, range.x.max + 1).map((cell) => cell.value));
  });

  readonly column$ = (props: ({ index: number } | Named) & { includeName?: boolean }) =>
    this.select((state) => {
      const x = isNamed(props)
        ? state.dataset[0].findIndex((cell) => cell.value == props.name)
        : props.index;
      if (x < 1) return [];
      return state.dataset.map((row) => row[x]).slice(props.includeName ? 0 : 1);
    });

  readonly row$ = (props: ({ index: number } | Named) & { includeName?: boolean }) =>
    this.select((state) => {
      const y = isNamed(props)
        ? state.dataset.findIndex((cell) => cell[0].value == props.name)
        : props.index;
      if (y < 1) return [];
  ...
  readonly write = this.updater((state, { value }: { value: string }) => {
    ...
    if (state.start) state.hasFocus = true;
    ...
    if (Ranges.equals(state.start, maxInColumn.coords)) {
      if (value.length > maxInColumn.value.length)
        state.maxCols[x] = { value, coords: state.start }; // Increasing maximum cell
      else state.maxCols[x] = Ranges.findMaximumOfColumn(state, x); // Decreasing maximum cell ==> searching for new maximum
    } else if (value.length > maxInColumn.value.length)
      state.maxCols[x] = { value, coords: state.start };

    return { ...state, dataset: [...state.dataset] };
  });

  readonly focus = this.updater((state) =>
    Cells.select({
      ...state,
      hasFocus: true,
    })
  ...
  readonly blur = this.updater((state) =>
    Cells.select({
      ...state,
      hasFocus: false,
    })
  ...
  readonly selectCell = this.updater(
    (state, { coords, shift }: { coords: Coords; shift?: boolean }) => {
      const origin = Ranges.origin(state);
  ...
  readonly selectRange = this.updater(
    (state, { start, stop }: { start?: Coords; stop?: Coords }) => {
      const origin = Ranges.origin(state);
  ...
  readonly paste = this.updater((state, { table }: { table: string[][] }) => {
    ...
    for (let yFrom = 0; yFrom < table.length; yFrom++) {
      ...
      for (let xFrom = 0; xFrom < table[yFrom].length; xFrom++) {
        ...
        if (state.dataset[yTo] && state.dataset[yTo][xTo]) {
          ...
          if (
            value.length > maxCol.value.length ||
            Ranges.equals(maxCol.coords, {
              x: xTo,
              y: yTo,
            })
          )
    ...
    state.stop = Ranges.limitCoords(
      { x: range.x.min + width(table), y: range.y.min + height(table) },
      state
    ); // Make selection range equal to the pasted region
    return Cells.select({ ...state, dataset: [...state.dataset], maxCols: [...state.maxCols] });
  });

  readonly clear = this.updater((state) => {
    const dataset =
      state.numberOfRows && state.numberOfColumns
        ? generateTableCell(state.numberOfColumns, state.numberOfRows)
        : [[cell()]];
    return {
      ...state,
      start: { x: 0, y: 0 },
  ...
  readonly init = this.updater(
    (
      ...
      if (!table && !(numberOfRows && numberOfColumns))
        throw new Error(
          'Needs no provide either a table, or a table template with numberOfColumns and numberOfRows'
        );
      if (numberOfRows && numberOfColumns) {
        const { renameCols, renameRows, importMapHeaders } = state.settings;
        this.settings({
          settings: { importMapHeaders: false, renameRows: true, renameCols: true },
        });
        this.import({
    ...
    (
      ...
      if (state.settings.importMapHeaders) {
        ...
        for (let yFrom = 0; yFrom < table.length; yFrom++) {
          ...
          } else if (fullImport || state.settings.addRow) {
            // New row to add
          ...
          for (let xFrom = 0; xFrom < table[0].length; xFrom++) {
            ...
            } else if (fullImport || state.settings.addColumn) {
              // New column to add
            ...
            const value = (table[yFrom][xFrom] || '').trim();
            state.dataset[yTo][xTo].value = value;
            if (value.length > state.maxCols[xTo].value.length)
              state.maxCols[xTo] = { value, coords: { x: xTo, y: yTo } };
          }
        }
      } else {
  ...
  readonly deleteRow = this.updater((state, props: { y: number } | Named) => {
    const y = isNamed(props)
      ? state.dataset.map((row) => row[0]).findIndex((cell) => cell.value === props.name)
      : props.y;
    if (y < 1) return state;
    ...
    state.maxCols
      ...
      .forEach(
        (max) => (state.maxCols[max.coords.x] = Ranges.findMaximumOfColumn(state, max.coords.x))
      ); // We find the new one

    return Cells.select({ ...state, dataset: [...state.dataset], maxCols: [...state.maxCols] });
  });

  readonly setting = this.updater(
    (state, { setting, value }: { setting: keyof Settings; value: boolean }) => ({
      ...state,
      settings: {
        ...state.settings,
        [setting]: value,
  ...
  readonly importFileContent = this.updater(
    (
      state,
      {
        content,
  ...
  readonly importFile = this.updater(
    (state, { file, replace }: { file: File; replace?: boolean }) => {
...
export namespace Ranges {
  ...
  export function limitRange(
    range: Range,
    state: TableState,
    limits = Ranges.limits(state)
  ): Range {
    return {
      x: {
        min: limit(range.x.min, limits.x.min, limits.x.max),
        max: limit(range.x.max, limits.x.min, limits.x.max),
      },
      y: {
        min: limit(range.y.min, limits.y.min, limits.y.max),
        max: limit(range.y.max, limits.y.min, limits.y.max),
      },
    };
  }
  ...
  export function findMaximumOfColumn(state: TableState, x: number): LocatedValue {
    return Ranges.column(state.dataset, x).reduce(
      (max, e, y) =>
        e.value?.length > max.value.length
          ? {
              value: e.value,
              coords: { x, y },
            }
