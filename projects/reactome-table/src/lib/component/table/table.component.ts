...
export class TableComponent implements OnInit, OnChanges, AfterViewInit, OnDestroy {
  ...
  startCell$: Observable<HTMLTableCellElement> = this.start$.pipe(
    ...
    tap((start) => {
      if (
        start.y < this.viewport().getRenderedRange().start ||
        start.y > this.viewport().getRenderedRange().end
      ) {
  ...
  startClasses$: Observable<Record<string | number, boolean>> = combineLatest(
    [this.startCell$, this.tableStore.hasFocus$],
    (start, hasFocus) =>
      Array.from(start?.classList || []).reduce((o, clazz) => ({ ...o, [clazz]: true }), {
        visible: hasFocus,
      } as Record<string | number, boolean>)
  );
  inputLevel$: Observable<number> = this.start$.pipe(
    map((start) => (start.x === 0 ? 6 : start.y === 0 ? 4 : 2))
  );
  ...
  ngOnInit(): void {
    ...
    this.startCoords$.pipe(delay(1), untilDestroyed(this)).subscribe(() => this.focusInput());
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['userSettings'] && !changes['userSettings'].firstChange)
      this.tableStore.settings({ settings: this.userSettings() });
  }
  ...
  mousemove($event: MouseEvent) {
    if (this.isDraggingMouse) {
      ...
      if (this.previousStop?.x !== stop.x || this.previousStop?.y !== stop.y)
        this.tableStore.selectRange({ stop });
  ...
  exportFile(): void {
    this.tableStore.cleanData$
      .pipe(first())
      .subscribe((table) => downloadTable(table, this.name()));
  }
