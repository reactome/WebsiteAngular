...
export const transpose = <T>(table: T[][]): T[][] =>
  table[0].map((_, colIndex) => table.map((row) => row[colIndex]));
export const findColumnValuesByName = <T>(table: T[][], columnHeader: T): T[] | undefined =>
  transpose(table)
    .find((col) => col[0] === columnHeader)
    ?.slice(1);

export const pushAll = <T>(table: T[][], value: T): number =>
  table.reduce((_, col) => col.push(cp(value)), 0);
...
export function generateTable(numberOfColumns: number, numberOfRows: number): string[][] {
  return new Array(numberOfRows).fill(0).map((_, y) =>
    new Array(numberOfColumns).fill(0).map((_, x) => {
      if (x === 0 && y === 0) return '';
      if (x === 0) return y + '';
