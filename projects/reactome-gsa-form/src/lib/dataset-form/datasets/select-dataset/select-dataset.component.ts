...
export class SelectDatasetComponent implements OnInit {
  ...
  constructor() {
    ...
    effect(() => {
      this.localSources$ = this.store.select(
        datasetSourceFeature.selectBySource('Local', this.method())
      );
      this.externalSources$ = this.store.select(
        datasetSourceFeature.selectBySource('External', this.method())
      );
      this.exampleSources$ = this.store.select(
        datasetSourceFeature.selectBySource('Example', this.method())
      );
    });
  }
