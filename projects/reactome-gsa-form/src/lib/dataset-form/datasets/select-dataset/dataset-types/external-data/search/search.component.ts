...
export class SearchComponent implements OnInit {
  ...
  searchStatus$: Observable<'pending' | 'finished' | 'waiting'> = this.store.select(
    searchResultFeature.selectSearchStatus
  );
  ...
  select(result: SearchResult): void {
    this.store.dispatch(
      datasetActions.load({
        id: this.datasetId(),
        resourceId: result.resource_loading_id,
        parameters: result.loading_parameters,
      })
    );
  }
}
