...
export class ParamDatasetComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.sampleGroups$ = this.store
      .select(datasetFeature.selectSampleGroups(this.datasetId()))
      .pipe(distinctUntilChanged(), share());

    this.parameters$ = this.store
      .select(datasetFeature.selectSummaryParameters(this.datasetId()))
      .pipe(
        distinctUntilChanged(),
        share(),
        combineLatestWith(this.sampleGroups$),
        map(([parameters, sampleGroups]) =>
          parameters
