...
export class StatisticalDesignComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.someSelected$ = this.store.select(
      datasetFeature.selectCovariancesSomeSelected(this.datasetId())
    );
    this.allSelected$ = this.store.select(
      datasetFeature.selectCovariancesAllSelected(this.datasetId())
    );
  }

  changeAllCovariates(value: boolean) {
    this.store.dispatch(datasetActions.setCovariatesValue({ id: this.datasetId(), value: value }));
  }
  ...
  setCovariance(value: boolean, group: string) {
    this.store.dispatch(
      datasetActions.setCovariateValue({
        group,
        value,
        id: this.datasetId(),
      })
