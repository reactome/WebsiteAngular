...
export const methodReducer: ActionReducer<MethodState> = createReducer(
  ...
  on(methodActions.setParams, (state, { methodName, parameters }) =>
  ...
  on(methodActions.resetParams, (state, { methodName }) =>
    helper.update(methodName, state, (method) => ({
      parameters: method.parameters.map((param) => ({ ...param, value: param.default })),
    }))
  ),
  on(methodActions.updateSelectedParams, (state, { parameters }) =>
    !state.selectedMethodName
      ? state
      : helper.update(state.selectedMethodName, state, (method) => ({
          parameters: method.parameters.map(
            (originalParam) =>
              parameters.find((param) => param.name === originalParam.name) || originalParam
          ),
        }))
  ),
  on(methodActions.updateSelectedParam, (state, { param }) =>
    !state.selectedMethodName
      ? state
      : helper.update(state.selectedMethodName, state, (method) => ({
          parameters: method.parameters.map((originalParam) =>
            originalParam.name === param.name ? param : originalParam
          ),
        }))
  ),
  on(methodActions.updateCommonParam, (state, { param }) => ({
    ...state,
    commonParameters: state.commonParameters.map((originalParam) =>
      originalParam.name === param.name ? param : originalParam
    ),
  })),
  on(methodActions.select, (state, { methodName }) => ({
    ...state,
