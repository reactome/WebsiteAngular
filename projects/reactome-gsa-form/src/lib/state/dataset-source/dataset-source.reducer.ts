...
export const datasetSourceReducer: ActionReducer<DatasetSourceState> = createReducer(
  initialState,
  on(datasetSourceActions.loadLocalsSuccess, (state, { locals }) =>
    datasetSourceAdapter.addMany(
      locals.map((local) => ({
        ...local,
        source: 'Local',
      })),
  ...
  on(datasetSourceActions.loadExamplesSuccess, (state, { examples }) =>
    datasetSourceAdapter.addMany(
  ...
  on(datasetSourceActions.loadExternalSuccess, (state, { externals }) =>
    datasetSourceAdapter.addMany(
  ...
  on(datasetSourceActions.setParameter, (state, { id, param }) =>
    helper.update(id, state, (source) => ({
      parameters: source.parameters?.map((originalParam) =>
