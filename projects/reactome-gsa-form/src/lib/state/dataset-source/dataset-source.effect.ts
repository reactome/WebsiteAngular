...
export class DatasetSourceEffects {
  ...
  loadLocals = createEffect(() =>
    this.actions$.pipe(
      ofType(datasetSourceActions.loadLocals),
      take(1),
      exhaustMap(() =>
        this.service.fetchLocalDataSources().pipe(
          map((datasetSources) =>
            datasetSourceActions.loadLocalsSuccess({ locals: datasetSources })
          ),
  ...
  loadExamples = createEffect(() =>
    this.actions$.pipe(
      ...
      take(1),
      exhaustMap(() =>
        this.service.fetchExampleDataSources().pipe(
          map((datasetSources) =>
            datasetSourceActions.loadExamplesSuccess({ examples: datasetSources })
          ),
  ...
  loadExternals = createEffect(() =>
    this.actions$.pipe(
      ofType(datasetSourceActions.loadExternal),
      take(1),
      exhaustMap(() =>
        this.service.fetchExternalDataSources().pipe(
          map((externals) => {
            externals.forEach((external) =>
              external.parameters.forEach((param) => {
