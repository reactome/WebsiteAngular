...
The [API](</ContentService/>) includes a set of methods classified in groups according to their functionality. For instance, expanding the [pathways group](</ContentService/#/pathways>) reveals a set of methods that provide specific information about pathways such as the contained Events or the participating PhysicalEntities. All methods are specified as either GET or POST requests. GET and POST are not interchangeable. [Expanding a particular method](</ContentService/#/participants/getParticipatingPhysicalEntities>) provides more information about its input and output parameters as well as a simple test case for users to try out. In addition, users can also use any command line client such as [wget](<http://www.gnu.org/software/wget/>) or [curl](<http://curl.haxx.se/docs/manual.html>) to access the methods.
...
As the last example in the section, let’s check out how to retrieve the participanting molecules of the reaction used in the previous examples. To do so, we call the [getParticipatingPhysicalEntities](</ContentService/#/participants/getParticipatingPhysicalEntities>) method in the API:
