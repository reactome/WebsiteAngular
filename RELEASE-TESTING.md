...
| A pathway's SBGN export                                                                 | **auto** — `e2e/release/sbgn-export.spec.ts`, against a pathway that is not one of our fixtures. _A pathway's SBGN is built from its diagram json on disk; those files were deleted and 9,548 of 9,559 pathways answered 500 for weeks, while the 11 our fixtures use kept working. A reaction needs no file, so the two are asserted separately_ |
