...
These four numbers are counted from the rows below. If they disagree with the
rows, the rows are right: this line has drifted twice from being edited by hand.
...
| Item                                                                                  | Status                                                                                                                                                                                                                                                                                                                                                                                                              |
| ------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Version number and release date at the bottom of the homepage                         | **auto** — `e2e/release/content-currency.spec.ts` compares it against `/data/database/version`                                                                                                                                                                                                                                                                                                                      |
| Every authored page has a real title                                                  | **auto** — `content-integrity.spec.ts`. _Digital Preservation was headed "UNTITLED"_                                                                                                                                                                                                                                                                                                                                |
| No internal link leads to a missing page                                              | **auto** — `check-links.spec.ts` reads every link from the HTML each page renders and resolves it against content files, routes and static files (and a section link against the ids that page renders), never by fetching (a missing page answers 200). Links waiting on something are listed with their reason in `KNOWN_BROKEN`. _Over a hundred were broken, counting links to sections, not the five reported_ |
| No image in content is a web page                                                     | **auto** — `check-links.spec.ts`; local images are judged by their bytes. _Three news items showed broken images this way_                                                                                                                                                                                                                                                                                          |
| Release-stamped figures show the current release                                      | **auto** — `content-pages.spec.ts` for the inferred-events chart; `release-placeholder.spec.ts` for `{release}` in both raw HTML and markdown                                                                                                                                                                                                                                                                       |
...
| News is current, and the links inside the latest news item work                       | **auto** — `e2e/release/content-currency.spec.ts` checks the newest announcement against the release being served. _Announcements are imported verbatim by `npm run import:news`; nothing generates their prose_                                                                                                                                                                                                    |
...
## Search page React-to-Me answer
...
| Item                                                             | Status                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ---------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Nothing is fetched until the reader clicks "Ask React-to-Me"     | **auto** — `search-answer.spec.ts` asserts the endpoint is called zero times before the click. A crawled search URL must not cost a model call                                                                                                                                                                                                                                                                                                                                                 |
| The answer streams in, with its sources as links                 | **auto** — `search-answer.spec.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| An answer with no sources renders no sources heading             | **auto** — `search-answer.spec.ts`. An answer with no citations is legitimate, not a failure, and the heading must not appear over an empty list. _Userguide questions cited nothing until React-to-Me began sending `url` citations on 18 Sep; they now cite their pages_                                                                                                                                                                                                                     |
...
| A human check is required before an answer is given              | **manual** — the Turnstile widget's first render is the one path no test exercises; the e2e stubs the exchange, so the real widget and the real identity cookie are both unexercised. Ask a question, confirm a challenge appears, then an answer. A second question in the same visit should not challenge again — **test this over HTTPS**: the identity cookie is `Secure`, so on a plain-HTTP dev server it is dropped and every question re-challenges, which looks like a bug and is not |
...
| About → Statistics shows the current release and date                  | **auto** — `e2e/release/content-currency.spec.ts`. _This was stale: the markdown embedded release 95's figures because the number was typed in once. Content asks for `{release}` now and the renderer substitutes it_ |
...
| Docs → Computational Inferred Events shows the current release's image | **auto** (the figure loads) — `e2e/release/content-currency.spec.ts`. _Whether the figure depicts the current release cannot be read off the image_                                                                    |
| Download menu: every link valid and every file present                 | **auto** — `e2e/release/download-files.spec.ts` HEADs all 74 offered files and asserts the release matches what the content service serves                                                                             |
...
| Exactly the 16 documented species in the dropdown                           | **auto** — `e2e/release/release-checklist.spec.ts`                                                                                                                                                                                                                                   |
| Every top-level pathway draws a diagram                                     | **auto** — `e2e/release/diagram-coverage.spec.ts` walks all 29 from the content service, so a pathway promoted to top level is covered the release it appears                                                                                                                        |
...
| An illustration is drawn whole, and one that cannot be drawn says so        | **auto** — `ehld-svg.spec.ts` parses a reduced Circadian clock as XML and checks nothing after its XHTML gradient is swallowed; `pathway-browser.spec.ts` serves a broken file and expects the message. _Illustrations went in as HTML, which drew Circadian clock as a blue square_ |
...
| A pathway's SBGN export                                                                 | **auto** — `e2e/release/sbgn-export.spec.ts`, against a different pathway every run. _A pathway's SBGN is built from its diagram json on disk; those files were deleted and 9,548 of 9,559 pathways answered 500 for weeks, while the 11 our fixtures use kept working. The exporter caches each pathway's SBGN after the first request, so a fixed id would be cached by its own first run and keep passing after the files vanished. A reaction needs no file, so the two are asserted separately_ |
...
| Item                                                                                         | Status                                                                                                                                                                                                                                                                                           |
| -------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Six tabs present and populated for a pathway                                                 | **auto** — `e2e/release/release-checklist.spec.ts` for presence, `detail-contents.spec.ts` for the contents of the four that need no analysis; the other two are covered by `analysis-results.spec.ts` once there is one                                                                         |
...
| An event's GO biological process is shown, linked to the term, and absent when it has none   | **auto** — `detail-contents.spec.ts` (Hemostasis: blood coagulation, GO:0007596)                                                                                                                                                                                                                 |
...
| Sharing an overlay by link                                      | **auto** — `custom-interactor-dialog.spec.ts`; the link opens the same overlay for someone else, and the resource is listed so they can clear it |
| Deleting a resource you added                                   | **auto** — `custom-interactor-dialog.spec.ts`, asserted on the diagram rather than on the list — the list emptying is what hid this being broken |
| An overlay survives navigating away and back                    | **auto** — `interactor-threshold.spec.ts`; counted in badges drawn, not in the address                                                           |
| The count beside a resource follows the pathway on screen       | **auto** — `interactor-counts.spec.ts`; a unit test on the cache decision, because a browser test of it passed with and without the fix          |
| Your selection survives opening a pathway from inside a diagram | **auto** — `diagram-behaviour.spec.ts`; asserted on `select` in the address, which is what survives a reload                                     |
| Interactors reachable without a mouse                           | **auto** — `header-controls.spec.ts`; focus, Enter/Space to open, Escape to close, and focus returns to the control                              |

## Analysis tools

| Item                                                                          | Status                                                                                                                                                                                                                                                                              |
| ----------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| All four tools reachable, each opening on its own tab                         | **auto** — `e2e/release/release-checklist.spec.ts`. _The old document notes the old site opened every tool on "Analyse gene list"; ours does not, and the test asserts that_                                                                                                        |
| Gene list analysis runs and returns results                                   | **auto** — `analysis.spec.ts`                                                                                                                                                                                                                                                       |
| Example and species names readable on a laptop screen                         | **auto** — `analysis.spec.ts` at 1366×768 and 1280×720, checking each name is neither cut nor outside its control                                                                                                                                                                   |
| Analysis options can be chosen from the keyboard                              | **auto** — `analysis.spec.ts`; each option is a checkbox that Space and Enter toggle                                                                                                                                                                                                |
| GSA methods load from GSAServer                                               | **auto** — `analysis.spec.ts`                                                                                                                                                                                                                                                       |
| A mistyped analysis email is refused with a message                           | **manual** — the cause is covered by `method-parameter.spec.ts`, but the rendered message is not: the component has an external `templateUrl`, which the vitest setup cannot compile. Type `adam@` on the options step and check the field explains itself and Continue is disabled |
...
| A result that cannot be loaded says so                                        | **auto** — `analysis-results.spec.ts` answers the token with 410 and expects the message. _It used to vanish without a word_                                                                                                                                                        |
| A summary only ever appears beside the result it describes, and can be closed | **auto** — `summary-owner.spec.ts` for the decision across a new panel, `analysis-results.spec.ts` for the reader's path: summarise, run another analysis, no trace of the first                                                                                                    |
...
| Illustrated pathways export the whole illustration                       | **auto** — `downloads.spec.ts`, for a light illustration and a heavy one. _The heavy case is the row that matters: every illustration test used Apoptosis, which is 502 SVG elements and arrives quickly, while 96 of the 218 illustrations came out as a 140×140 picture of the zoom control_ |
...
| Logo page: every option saves a file of the size it states               | **auto** — `downloads.spec.ts` checks each of the 16 files' bytes and reads the PNG header against the stated pixel size                                                                                                                                                                       |
